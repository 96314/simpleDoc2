...
import core.ast.Comments;
import core.ast.ResolvedTypes;
import core.ast.Tag;
import core.schema.Group;
import core.schema.Node;
    ...
    public void accept(Comments comments) {
        ...
        for (Tag tag : comments.getTags()) {
            if ("return".equals(tag.getName()) && !Strings.isNullOrEmpty(tag.getContent())) {
                SymbolReference<ResolvedReferenceTypeDeclaration> symbolReference = ApiContext.getContext().getEnv().getTypeSolver().tryToSolveType(tag.getContent());
